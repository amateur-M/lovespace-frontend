...
function statusMeta(status: number): { className: string; label: string } {
  switch (status) {
    ...
    default:
      return {
        className: 'border border-rose-200 bg-rose-100 text-rose-900',
        label: `状态 ${status}`,
      }
  }
}
  ...
  return (
    <Card className={`ls-surface !shadow-sm ${className ?? ''}`} styles={{ body: { padding: 24 } }}>
      <div className="mb-4 flex justify-center">
        <Tag
          bordered={false}
          className={`!m-0 !rounded-full !px-3 !py-0.5 !text-xs !font-medium ${meta.className}`}
        >
          {meta.label}
        </Tag>
      </div>
      <Space size={32} align="start" className="w-full justify-center" wrap>
        <Space direction="vertical" align="center" size={8}>
          <Avatar size={80} src={resolveMediaUrl(me?.avatarUrl) || undefined}>
        ...
        <Space direction="vertical" align="center" size={8}>
          <Avatar size={80} src={resolveMediaUrl(partner?.avatarUrl) || undefined}>
